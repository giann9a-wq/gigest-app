...
export async function POST(request: NextRequest) {
  ...
  if (!name) {
    return NextResponse.json({ error: "Il nome commessa e obbligatorio" }, { status: 400 });
  }
  ...
  const dashboard = await getJobOrderDashboard(created.id);

  if (!dashboard) {
    return NextResponse.json({ error: "Commessa creata ma dashboard non disponibile" }, { status: 500 });
  }
